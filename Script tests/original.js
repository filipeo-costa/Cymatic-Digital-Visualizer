...
const audioFile = './Get it by your hands.mp3'; // change name path only
...
const analyser = new THREE.AudioAnalyser(sound, 128); // higher FFT size for detail
...
const uniforms = {
  ...
  u_green: { value: 1.0 },
  u_blue: { value: 1.0 },
  u_waveType: { value: 0 }, // 0=Sine, 1=Square, 2=Triangle, 3=Sawtooth
};
...
// Vertex shader (with selectable waveform)
const vertexShader = `
...
// waveform helper: returns value in [-1,1]
float wave(float x) {
  if (u_waveType == 0) {
    return sin(x); // Sine
    ...
    // Triangle via arcsin(sin(x)) scaled to [-1,1]
    return (2.0 / 3.14159265) * asin(sin(x));
  } else if (u_waveType == 3) {
    // Sawtooth: ramp in [-1,1] with period 2PI
...
// Particle shaders (match waveform for consistency)
...
void main() {
  float freq = u_frequency * 0.02; 
  float displacement = wave(position.y * 5.0 + u_time * 3.0) * freq; 
...
function waveJS(x, type) {
  if (type === 0) return Math.sin(x); // sine
  if (type === 1) return Math.sign(Math.sin(x)); // square
  if (type === 2) return (2 / Math.PI) * Math.asin(Math.sin(x)); // triangle
  if (type === 3) { const t = fract(x / (2 * Math.PI)); return 2 * t - 1; } // sawtooth
  return Math.sin(x);
}
...
// Mouse move for camera smooth follow
